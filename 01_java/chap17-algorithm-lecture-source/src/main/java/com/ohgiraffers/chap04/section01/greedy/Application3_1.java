...
public class Application3_1 {
    ...
    public static int solution(String input) throws Exception {
        ...
        /* 설명.
         *  Greedy -> 하나의 회의실에서 가장 많은 회의를 열기 위해서는 종료시간 순(오름차순)으로 고민하는 것이 유리
         *  = 빨리 끝나는 회의 먼저 고려
        * */
        ...
        Arrays.sort(time, (o1, o2) -> {
            ...
            if (o1[1] == o2[1]) {
                ...
                /* 설명. 시작 시간이 늦은 순서(내림차순)로 정렬 기준 정함 -> 회의 시간이 짧아지는 순서로 쌓기 위함 */
                return o2[0] - o1[0];
            }
            /* 설명. 종료시간이 같지 않을 때는 종료 시간이 이른 회의부터 정렬되도록 기준 잡음 (종료시간 오름차순) */
            return o1[1] - o2[1];
        });
